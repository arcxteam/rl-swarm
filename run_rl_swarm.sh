...
if [ "$CONNECT_TO_TESTNET" = true ]; then
    ...
    if [ -z "$DOCKER" ]; then
        if open http://localhost:5000 2> /dev/null; then
            echo_green ">> Successfully opened http://localhost:5000 in your default browser."
        else
            echo ">> Failed to open http://localhost:5000. Please open it manually."
        fi
    else
        echo_green ">> Please open http://localhost:5000 in your host browser."
    fi
    ...
    while true; do
        STATUS=$(curl -s "http://localhost:5000/api/get-api-key-status?orgId=$ORG_ID")
