...
# AUTO-PATCH GENRL FOR FLEXIBLE WANDB MODE
MLLOGGER_FILE="$ROOT/.venv/lib/python3.10/site-packages/genrl/logging_utils/ml_logger.py"
if [ -f "$MLLOGGER_FILE" ]; then
    # Check if already patched
    if grep -q 'mode="offline"' "$MLLOGGER_FILE" 2>/dev/null; then
        echo_green ">> Patching GenRL ml_logger for flexible W&B mode..."
        
        # Backup original
        cp "$MLLOGGER_FILE" "${MLLOGGER_FILE}.backup" 2>/dev/null || true
        
        # Create Python script to patch the file
        python3 << 'PYTHON_PATCH_EOF'
import re

ml_logger_path = "/root/rl-swarm/.venv/lib/python3.10/site-packages/genrl/logging_utils/ml_logger.py"

try:
    with open(ml_logger_path, "r") as f:
        content = f.read()

    # Pattern to match the hardcoded offline wandb.init block
    old_pattern = r'elif self\.log_with == LogTypes\.WANDB:\s+self\.tracker = wandb\.init\(\s+project="genrl",\s+dir=logging_dir,\s+mode="offline"\s+\)'

    # New flexible code
    new_code = '''elif self.log_with == LogTypes.WANDB:
            import os
            # Smart mode: online if credentials present, else offline (safe fallback)
            wandb_mode = os.getenv('WANDB_MODE', 'offline')
            wandb_api_key = os.getenv('WANDB_API_KEY', '')
            wandb_entity = os.getenv('WANDB_ENTITY', '')
            wandb_project = os.getenv('WANDB_PROJECT', 'genrl')
            
            # Safe fallback: if mode is online but credentials incomplete, use offline
            if wandb_mode == 'online' and (not wandb_api_key or not wandb_entity):
                wandb_mode = 'offline'
            ...
            self.tracker = wandb.init(**init_kwargs)'''

    # Apply patch
    content_patched = re.sub(old_pattern, new_code, content, flags=re.DOTALL)
    
    if content_patched != content:
        with open(ml_logger_path, "w") as f:
            f.write(content_patched)
        print("SUCCESS: GenRL ml_logger patched for flexible W&B mode")
    else:
        print("WARNING: Pattern not found, file may already be patched or have different format")

except Exception as e:
    print(f"ERROR patching ml_logger: {e}")
    print("Training will continue with default offline mode")
PYTHON_PATCH_EOF
        
        echo_green ">> GenRL patch complete. W&B mode will respect .env configuration"
    else
        echo_green ">> GenRL already patched or has custom configuration"
    fi
fi
# ===== END GENRL PATCH =====
